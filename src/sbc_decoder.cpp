...
static void synthesize8(int32_t* v, uint8_t* offset, int32_t* src, int16_t* dst)
{
	int i;
...
int sbc_decoder(SBC_Decode* sbc, const uint8_t *src, int src_len, void *dst, int dst_len, int *decoded)
{
	...
	if (decoded)
		*decoded = sbc->blocks * sbc->subbands * sbc->channels * 2;
	return framelen;
}
